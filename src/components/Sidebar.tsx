...
const Sidebar = () => {
  ...
  const { getPlaybackHistory } = useMusicPlayer();

  // Get recently played count
  const recentlyPlayedCount = useMemo(() => {
    return getPlaybackHistory().length;
  }, [getPlaybackHistory]);
  ...
  const libraryItems = [
    { name: "All Songs", icon: Music, count: allTracks.length, onClick: () => navigate("/library") },
    { name: "Recently Played", icon: List, count: recentlyPlayedCount, onClick: () => navigate("/profile", { state: { tab: 'recent' } }) },
  ];
